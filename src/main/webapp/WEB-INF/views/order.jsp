			...
			<div class="content_area">
				...
				<div class="orderProduct_div">
					...
					<div class="product_kind_div">
						주문상품
						<span class="product_kind_div_kind"></span>종
						<span class="product_kind_div_count"></span>개
					</div>
					<!-- 상품 테이블 -->
					...
					<table class="product_table">
						...
						<tbody>
							<c:forEach items="${orderList}" var="ol">
								<tr>
									<td>
										<div class="image_wrap" data-id="${ol.imageList[0].id}"
									...
									<td class="product_table_price_td">
										<fmt:formatNumber value="${ol.salePrice}"
														  pattern="#,### 원" /> | 수량 ${ol.count}개 <br>
										<fmt:formatNumber value="${ol.totalPrice}"
														  pattern="#,### 원" /> <br>[
										<fmt:formatNumber value="${ol.totalPoint}"
														  pattern="#,### 원" />P]
										<input type="hidden" class="individual_name" value="${ol.name}">
										...
										<input type="hidden" class="individual_count_input" value="${ol.count}">
										<input type="hidden" class="individual_totalPrice_input" value="${ol.salePrice * ol.count}">
					...
					<div class="total_info_price_div">
						<ul>
							...
							<li class="point_li">
								...
								<input class="member_money" hidden="hidden" value="${member.money}">
							</li>
						</ul>
					</div>
					<!-- 버튼 영역 -->
...
function setTotalInfo(){
	...
	console.log("totalPrice_span :"+totalPrice);
	// 총 갯수
	$(".product_kind_div_count").text(totalCount);
	console.log("product_kind_div_count :"+totalCount);
	// 총 종류
	$(".product_kind_div_kind").text(totalKind);
	console.log("product_kind_div_kind :"+totalKind);
	// 총 마일리지
	$(".totalPoint_span").text(totalPoint.toLocaleString());
	console.log("totalPoint_span :"+totalPoint);
	// 배송비
	$(".delivery_price_span").text(deliveryPrice.toLocaleString());
	console.log("delivery_price_span :"+deliveryPrice);
	// 최종 가격(총 가격 + 배송비)
	$(".finalTotalPrice_span").text(finalTotalPrice.toLocaleString());
	console.log("finalTotalPrice_span :"+finalTotalPrice);
	// 할인가(사용 포인트)
	$(".usePoint_span").text(usePoint.toLocaleString());
	console.log("usePoint_span :"+usePoint);
...
$(".order_btn").on("click", function(){
	...
	$(".product_table_price_td").each(function(index, element){
		...
		let name_input = "<input name='orders[" + index + "].name' type='hidden' value='" + name + "'>";
	...
	if(money < finalTotalPrice == true){
		alert('금액이 부족합니다 금액을 충전해주세요.');
	} else{
