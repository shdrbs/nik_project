	...
	<div class="wrap">
		...
		<div class="content_area">
						...
						<c:forEach items="${cartInfo}" var="ci">
							<tr>
								<td class="td_width_1 cart_info_td">
									...
									<input type="hidden" class="individual_count_input" value="${ci.count}">
									<input type="hidden" class="individual_totalPrice_input" value="${ci.salePrice * ci.count}">
									<input type="hidden" class="individual_point_input" value="${ci.point}">
									<input type="hidden" class="individual_totalPoint_input" value="${ci.totalPoint}">
									<input type="hidden" class="individual_id_input" value="${ci.id}">
								</td>
								<td class="td_width_2">
									<div class="image_wrap" data-id="${ci.imageList[0].id}"
								...
								<td class="td_width_4 price_td">
									<del>
										정가 : <fmt:formatNumber value="${ci.price}" pattern="#,### 원" />
									</del><br>
									판매가 :
									<span class="red_color"><fmt:formatNumber
											value="${ci.salePrice}" pattern="#,### 원" />
									</span><br>
									마일리지 :
									<span class="green_color"><fmt:formatNumber
											value="${ci.point}" pattern="#,###" />
									</span>
								</td>
								<td class="td_width_4 table_text_align_center">
									<div class="table_text_align_center quantity_div">
										<input type="text" value="${ci.count}" class="quantity_input">
										<button class="quantity_btn plus_btn">+</button>
										<button class="quantity_btn minus_btn">-</button>
									</div>
									<a class="quantity_modify_btn btn btn-primary" data-cartid="${ci.cartId}">변경</a>
								</td>
								<td class="td_width_4 table_text_align_center">
									<fmt:formatNumber value="${ci.salePrice * ci.count}" pattern="#,### 원" />
								</td>
								<td class="td_width_4 table_text_align_center">
									<button class="delete_btn btn btn-danger" data-cartid="${ci.cartId}">삭제</button>
								</td>
							</tr>
						</c:forEach>
					</tbody>
			...
			<div class="content_btn_section">
				<a class="btn btn-primary order_btn">주문하기</a>
			</div>
...
function setTotalInfo(){
	...
	// each(index, element) : 매개변수로 받은 것을 for in반복문과 같이 배열이나 객체의 요소를 검사하는 메서드
	$(".cart_info_td").each(function(index, element){

		if($(element).find(".individual_cart_checkbox").is(":checked") === true){	//체크여부
			// 총 가격
			// 접근한 <td> 객체에 있는 'individual_totalPrice_input' <input> 태그에 값을 totalPrice 변수의 값에 더해준다
			// 모든 상품 <td> 객체를 순회하게 되면 totalPrice는 모든 상품의 가격이 더해져서 '총 상품 가격'이 구해지게 된다
			// parseInt()를 사용한 이유는 <input> 태그의 값을 얻어오면 그 값은 'string' 타입으로 인식이 되어서 이를 int 타입으로 변경해주기 위함
	...
	/* 값 삽입 */
	// 총 가격
	// toLocaleString() : Number형식 을 196,000이런식으로 ','구분자를 사용해 변환해준다
...
$(".order_btn").on("click", function(){
	...
	if (form_contents == '' || form_contents == null) {
